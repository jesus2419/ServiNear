...
import java.io.ByteArrayOutputStream
    ...
    class MostrarchatsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_mostrarchats, container, false)
    ...
    private fun filterServicios(query: String?) {
        ...
        private fun obtenerChatsDesdeServidor() {
            val url = "http://74.235.95.67/api/obtener_chats.php"
            val queue: RequestQueue = Volley.newRequestQueue(requireActivity())
            val usuario2 = userManager.getUser()?.username ?: return  // Obtener el nombre de usuario de UserManager

            val stringRequest = object : StringRequest(
                    Method.POST, url,
                    Response.Listener { response ->
                        try {
                            Log.d("Response", "Respuesta del servidor: $response") // Añadir registro de la respuesta
                            val jsonArray = JSONArray(response)
                            if (jsonArray.length() > 0) {
                                Log.d("Chats", "Número de chats recibidos: ${jsonArray.length()}")
                                for (i in 0 until jsonArray.length()) {
                                    val chat = jsonArray.getJSONObject(i)
                                    //val chatID = chat.getString("ChatID")
                                    //val contenido = chat.getString("contenido")
                                    //val fechaDeCreacion = chat.getString("fecha_de_creacion")
                                    //val estado = chat.getString("estado")
                                    //val remitente = chat.getString("Remitente")
                                    //val destinatario = chat.getString("Destinatario")
                                    //val idDestino = chat.getString("id_destino")
                                    //val imagenBase64 = chat.getString("foto")
                                    val idDestino = chat.getString("ID")
                                    val usuarioo = chat.getString("usuario")
                                    val Nombre = chat.getString("Nombre")
                                    val imagenBase64 = chat.getString("Foto")
                                    ...
                                    if (imagenBitmap != null) {
                                        // Mostrar el chat en la interfaz
                                        mostrarChat(usuarioo, idDestino, Nombre, imagenBitmap)
                                    } else {
                                        Log.e("DecodeError", "Error al decodificar la imagen base64")
                                    }
                                }
                            } else {
                                showErrorToast("No se encontraron chats")
                                // Intentar cargar chats locales si no hay datos remotos
                                // cargarChatsLocales()
                            }
                        } catch (e: JSONException) {
                            e.printStackTrace()
                            showErrorToast("Error al procesar la respuesta del servidor")
                            Log.d("ChatActivity", "Username: $usuario2")
                            Log.d("ChatActivity", "id: $e")
                        }
                    },
                    Response.ErrorListener { error ->
                        Log.e("VolleyError", "Error en la solicitud: ${error.message}")
                        handleVolleyError(error)
                    }
            ) {
                override fun getParams(): Map<String, String> {
                    val params = HashMap<String, String>()
                    params["usuario"] = usuario2
                    Log.d("Params", "Parámetros enviados: $params") // Añadir registro de los parámetros
                    return params
                }
            }
        ...
        //destinatario, idDestino, contenido, fechaDeCreacion, imagenBitmap
        private fun mostrarChat(destinatario: String, idDestino: String, contenido: String, imagen: Bitmap) {
            ...
            servicioLayout.setOnClickListener {
                ...
                intent.putExtra("USERNAME", destinatario)
                intent.putExtra("id", idDestino)
                startActivity(intent)
            ...
            nombreTextView.text = destinatario
            ...
            descripcionTextView.text = contenido
